 ...
 * <div> (sin role="button"), por eso los clics van por texto visible scoped a
 * la tarjeta del cliente (evita chocar con los chips de filtro de estado).
 *
...
test.describe('Flujos críticos (usuario aislado)', () => {
  test('Login → Recepción TRM-XXXX → Estado → Cobro → Admin', async ({ page }) => {
    ...
    await page.getByText('Cambio de pantalla', { exact: true }).first().click();
    ...
    await page.getByText(/registrar recepción/i).first().click();
    const dialog1 = await dialogPromise;
    expect(dialog1.message()).toContain('Equipo recibido');
    ...
    await page.getByText(/ver recibo/i).first().click();
    ...
    const statusBtn = (label: string) => jobCard(page).getByText(label, { exact: true }).last();
    ...
    await jobCard(page).getByText(/registrar pago/i).click();
    const payDialogPromise = page.waitForEvent('dialog', { timeout: 20_000 });
    await jobCard(page).getByText(/^cobrar$/i).click();
    const payDialog = await payDialogPromise;
    expect(payDialog.message()).toContain('Pago registrado');
    await payDialog.accept();
