...
- **Acceso Read-Only a Inventario para Técnicos (2026-08-26):** Se habilitó la pestaña de inventario para que los técnicos puedan consultar el stock de repuestos sin privilegios de edición (crear/ajustar stock). La UI (`inventory.tsx` y `app-tabs.tsx`) bloquea controles de mutación y el RLS actual permite los `SELECT` correspondientes.
