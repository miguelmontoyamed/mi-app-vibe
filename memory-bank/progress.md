...
| Fecha | Cambio |
|-------|--------|
| 2026-08-25 | **Módulo de Compra y Venta de Equipos (Trade-in & Refurbished Devices):** Nuevo módulo con tab dedicado (`devices.tsx`) para gestionar compras de celulares a distribuidores (IMEI, costo, garantía proveedor) y ventas directas a clientes (utilidad neta, garantía cliente, método de pago). Factura comercial y comprobante de garantía con PDF en web (jsPDF) y nativo (`expo-print`/`expo-sharing`) con botón para compartir por WhatsApp. Aislamiento contable 100% independiente del servicio técnico (`repairs`). Suite de tests 122/122 PASS. |
