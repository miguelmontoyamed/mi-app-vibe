...
- **Blindaje y Asociación de Invitación de Técnicos (2026-08-27):**
  ...
  - Creación, aplicación automatizada y registro de la función RPC `claim_workshop_invitation(p_workshop_id)` y política de eliminación de huérfanos en Supabase (`20260828000000_technician_invite_link_claim.sql` y `schema.sql`).
  - Verificación de 9/9 checks en la base de datos real con `scripts/verify-invitation-pipeline.mjs` al 100% PASS.
  - Suite de pruebas unitarias al 100% PASS (143/143) y 0 errores de TypeScript (`npx tsc --noEmit`).
...
| Fecha | Cambio |
|-------|--------|
| 2026-09-09 | **Blindaje de Ciclo de Vida y Recontratación de Técnicos:** 1) Bloqueo de técnicos soft-deleted (`is_active = false`) en hidratación de sesión, login con contraseña y Google OAuth en `auth-context.tsx` y `login.tsx`, cerrando sesión automáticamente con aviso al usuario. 2) Corrección del bug de recontratación en la RPC `claim_technician_invitation` (`20260909210000_reactivate_technician_on_invitation_claim.sql` y `schema.sql`) asegurando `is_active = true` al aceptar invitaciones. Pipeline de verificación 100% OK. |
